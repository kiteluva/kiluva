...
pdf("J-K-spotify_analysis_all_models_plots.pdf", width = 12, height = 10)
...
plot4 <- ggpairs(spotify_modelling, columns = 16:21) # Adjusted column range
...
anova_importance <- data.frame(
  Feature = sapply(anova_results, function(x) x$Feature), # Extract Feature names.
...
cat("Random Forest - R-squared (R²):", R_squared_rf, "\n")
...
cat("All plots, including the model comparison and actual vs predicted plots,
    have been saved to J-K-spotify_analysis_all_models_plots.pdf in your working directory.\n")